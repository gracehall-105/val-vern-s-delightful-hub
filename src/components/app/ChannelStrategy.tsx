...
const CHANNEL_DESCRIPTIONS: Record<string, string> = {
  official: 'Company websites, product pages, and branded portals that AI models cite when naming providers.',
  publication: 'Third-party editorial content — blogs, news, comparison sites, educational media that AI uses for credibility.',
  government: 'Regulatory filings, .gov pages, and compliance databases that ground factual claims.',
  forum: 'Reddit, professional communities, review sites — user-generated signals AI models weigh for social proof.',
  social: 'LinkedIn, X/Twitter, and other social platforms where brand presence amplifies visibility.',
};
...
export function ChannelStrategy() {
  ...
  // Channel breakdown sorted by citation count
  const channels = useMemo(() => {
    if (!data) return [];
    return Object.entries(data.type_breakdown)
      .filter(([type]) => type !== '') // skip empty type
      .sort((a, b) => b[1] - a[1])
      .map(([type, citations]) => ({
        type,
  ...
  // Top sources per channel with Voya presence info
  const sourcesByChannel = useMemo(() => {
    ...
    // Each channel already sorted by citations (API returns sorted), take top 15
    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <Panel>
          ...
          <p className="mt-1 text-2xl font-display text-amber-600">{data.total_sources > 0 ? Math.round(((data.total_sources - data.voya_sources) / data.total_sources) * 100) : 0}%</p>
          ...
          {channels.map(ch => {
            ...
            return (
              <div key={ch.type}>
                ...
                >
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-3 text-xs text-muted-foreground">
                      ...
                      {presence && (
                        <span className={`flex items-center gap-1 ${presence.pct > 5 ? 'text-emerald-600' : 'text-amber-600'}`}>
                          {presence.pct > 5
                ...
                {isExpanded && (
                  <div id={`channel-details-${ch.type}`} className="mt-3 ml-5 pl-4 border-l-2 border-border space-y-3 animate-in fade-in slide-in-from-top-2 duration-200">
                    <p className="text-xs text-muted-foreground leading-relaxed">
                      {CHANNEL_DESCRIPTIONS[ch.type] || ''}
                    </p>

                    {/* Top sources in this channel */}
                    <div>
      ...
      <Panel className="mt-6" title="Strategic Recommendations" hint="Based on citation patterns">
        <div className="grid md:grid-cols-2 gap-5">
          {/* High-impact channels where Voya is underrepresented */}
          <div>
            ...
            <div className="space-y-2">
              {channels
                ...
                .map(ch => (
                  <div key={ch.type} className="p-3 rounded-lg bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800">
                    <p className="text-sm font-medium">{ch.label}</p>
                    <p className="text-xs text-muted-foreground mt-1">
                      {ch.citations.toLocaleString()} citations, but Voya appears in only {voyaPresence[ch.type]?.pct || 0}% of sources.
                      High-volume channel with visibility gap.
                    </p>
                  </div>
                ))
              }
              {channels.filter(ch => {
                ...
                <p className="text-xs text-muted-foreground">All high-volume channels have adequate Voya presence.</p>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-1.5">
              {data.sources
                ...
                .slice(0, 8)
                .map((src, i) => (
                  <div key={`${src.source_name}-${i}`} className="flex items-center justify-between text-xs py-1.5 px-2 rounded hover:bg-secondary/60">
                    <span className="truncate">{src.source_name}</span>
                    <span className="text-muted-foreground shrink-0 ml-2">{src.citations} cites across {src.prompts} prompts</span>
                  </div>
                ))
              }
            </div>
